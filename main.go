...
func main() {
	...
	if *waDebug != "" {
		...
		container, err = sqlstore.New(ctx, "sqlite", "file:"+exPath+"/dbdata/main.db?_pragma=foreign_keys(1)&_busy_timeout=3000", dbLog)
	} else {
		container, err = sqlstore.New(ctx, "sqlite", "file:"+exPath+"/dbdata/main.db?_pragma=foreign_keys(1)&_busy_timeout=3000", nil)
	}
	if err != nil {
		panic(err)
	}
